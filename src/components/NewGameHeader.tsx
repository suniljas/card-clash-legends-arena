...
const ResourceCounter = memo<{
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.05, y: -2, transition: { duration: 0.2, type: 'spring', stiffness: 300 } }}
      whileTap={{ scale: 0.95 }}
      title={label}
    >
      <div
      ...
      <motion.div
        ...
        animate={isAnimating ? { scale: [1, 1.3, 1], rotate: [0, 180, 360] } : {}}
        transition={{ duration: 0.5 }}
      >
        <Icon className={`w-4 h-4 relative z-10 transition-all duration-300 group-hover:scale-110`} style={{ color }} />
        <div
...
export function NewGameHeader() {
  ...
  const { gameStats, isAuthenticated, isSyncing } = useGameState();
        ...
        <div className="flex justify-between items-center">
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              ...
              <CloudSaveIndicator isAuthenticated={isAuthenticated} isSyncing={isSyncing} className="text-xs" />
            </div>
          </motion.div>
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            >
              <ResourceCounter icon={Coins} value={gameStats.coins} label={t('shop.coins')} color="#f59e0b" glowColor="#f59e0b33" />
              <ResourceCounter icon={Gem} value={gameStats.gems} label={t('shop.gems')} color="#3b82f6" glowColor="#3b82f633" />
              <ResourceCounter icon={Trophy} value={gameStats.pvpWins} label={t('leaderboards.battles_won')} color="#10b981" glowColor="#10b98133" />
              <ResourceCounter icon={Zap} value={gameStats.totalCards} label={t('leaderboards.cards_collected')} color="#8b5cf6" glowColor="#8b5cf633" />
            </motion.div>
